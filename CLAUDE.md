...
本プロジェクトは Automotive SPICE 4.0(PAM v4.0)の V字モデルに準拠する。
プロセス名称は 4.0 の定義に従う(3.1 の「適格性確認テスト」「受け入れテスト」は使わない)。
SWE.2.1 / 2.2 および SWE.5.1 / 5.2 の細分はプロジェクト独自の記法である。
...
| SWE.4 | ソフトウェアユニット検証 | `docs/common/60_unit_verification_design.md`(段階2.5-3で作成予定) |
| SWE.5.2 | ソフトウェアコンポーネント検証 | `docs/common/70_component_verification_design.md`(段階2.5-2で作成予定) |
| SWE.5.1 | 統合検証 | `docs/common/80_integration_verification_design.md`(段階2.5-1で作成予定) |
| SWE.6 | ソフトウェア検証 | `docs/common/90_software_verification_design.md`(段階2.5-0-4で作成予定) |
...
対応する 2 文書の番号の和は 110 になる(20↔90、30↔80、40↔70、50↔60)。
採番と版の選定の根拠は ADR-0006 を参照。
