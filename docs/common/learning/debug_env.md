...
## UART ログの確認（段階 2-D-3-2 以降）

クロック初期化後は UART0 で実行中の値を文字として出力できる。LED と gdb に加わる
3 つ目の観測手段で、以降のデバッグの主力になる。

### 配線

Pico の物理ピン 1〜3 と、Debug Probe の **`U`（UART）コネクタ**を繋ぐ。
SWD で使う `D` コネクタとは別なので注意する。

| Pico 物理ピン | 名称 | 向き | Debug Probe |
|---|---|---|---|
| 1 | GP0（UART0 TX） | → | RX |
| 2 | GP1（UART0 RX） | ← | TX |
| 3 | GND | — | GND |

TX と RX は交差して繋ぐ。GP0 / GP1 / GND が基板の角に連続して並んでいるため、
3 本まとめて取り出せる。

### 端末を開く

Debug Probe は USB シリアルとしても見える。ポートを確認して接続する。
...
複数のポートが見える場合、片方は CMSIS-DAP（デバッグ用）なので、
反対側を試す。

### 起動メッセージを見るには

起動時のクロック情報は電源投入直後に一度だけ出力されるため、端末を後から
開くと見逃す。**端末を先に開いてから、別のターミナルでリセットをかける**。
...
なお本プロジェクトのファームは 10 秒ごとにクロック情報を再表示するため、
そのまま待っていても状態を確認できる。

### 期待される出力

```text
=== RP2040 temp monitor (stage 2-D-3-2) ===
XOSC     : 12000 kHz
clk_ref  : 12001 kHz
clk_sys  : 125000 kHz
clk_peri : 125000 kHz
...
uptime が 1 秒刻みでずれずに進むことが、TIMER の 1 マイクロ秒刻みが
水晶精度で動いていることの確認になる。

### 終了手順（重要）

`screen` は**必ず kill で終了する**。

```text
Ctrl+A  →  K  →  y
```

`Ctrl+A` を押して離してから `K` を押し、確認に `y` と答える。

**`Ctrl+A` → `D` は使わない**。これはデタッチであり、プロセスが生き残って
シリアルポートを掴み続けるため、次回の接続時に「ビジー」で開けなくなる
（実際に踏んだ）。ウィンドウを閉じるだけの操作も同じ結果になる。

### ビジーになったときの復旧

```bash
screen -ls                       # 残っているセッションを確認
screen -X -S <セッション名> quit  # 個別に終了
pkill screen                     # まとめて終了
```

それでも解放されない場合は、ポートを掴んでいるプロセスを特定して終了する。

```bash
lsof /dev/tty.usbmodem<番号>
kill <PID>
```

### screen 以外の選択肢

キーバインドが煩わしい場合は `cat` でも読める（`Ctrl+C` で終了）。

```bash
stty -f /dev/tty.usbmodem<番号> 115200
cat /dev/tty.usbmodem<番号>
```

VS Code のシリアルモニタ拡張も使える。GUI で開始・停止できるため、
ポートの掴みっぱなしが起きにくい。

## Cortex-M0+ には除算命令が無い

UART で数値を 10 進表示しようとして判明した制約。
...
Cortex-M0+（ARMv6-M）には除算命令が存在せず、C の `/` や `%` は
コンパイラのランタイム関数呼び出しに変換される。それらは libgcc や
compiler-rt に含まれるが、本プロジェクトは `-nostdlib` でリンクしているため
未定義になる。

対処は 2 通りある。

1. libgcc をリンクする — 外部ランタイムへの依存が増える
2. 除算を使わない実装にする — **本プロジェクトで採用**

`uart_put_u32()` は 10 のべき乗のテーブルを用意し、「何回引けるか」を数える
方式で桁を求めている。減算と比較だけで済むため、ランタイム関数を必要としない。

ただし **2 の冪による除算・剰余は問題ない**。コンパイラがシフトと
ビット論理積に変換するためである。
...
定数同士の除算（例: ボーレート分周比の計算）もコンパイル時に畳み込まれるため、
実行時の除算は発生しない。

リンクが通っても油断せず、次のコマンドで確認できる。

```bash
arm-none-eabi-nm build/firmware.elf | grep -iE "aeabi|udiv|idiv"
```
