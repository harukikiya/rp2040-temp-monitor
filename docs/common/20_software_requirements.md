...
:layer: driver
:refines: SYS_001
:open_params: TBD-010 I2C速度は仮置きで100kHz、LCDの応答性を確認後に決定。解決時期: 段階3-2。
...
:open_params: TBD-012 閾値0.3℃は仮置き、実機での室温変動の観察結果に基づき調整する（SYS_003 の TBD-005 を統合）。解決時期: 段階4-4。
...
:layer: service
:refines: SYS_004
:rationale: 具体的な通信機構は ADR-0002 で SIO FIFO を採択済み（TBD-014 解決）。送受信関数の規定は SWR_027 が持つ。
...
:layer: service
:refines: SYS_004
:rationale: API の形は ADR-0002 の決定に基づき SWR_028 が規定済み（TBD-015 解決）。構造体を引数に取り、内部で SIO FIFO のワード列に分解・組み立てを行う。
