...
:tbd_items: TBD-019 連続失敗数のリセット条件（表示要求の成功時にリセットするか、再初期化の完了時にリセットするか）は未定。4回失敗した後に1回失敗し再び失敗した場合の遷移が定まらず、閾値の境界を検証できない。欠番である SWR_032 との関係を含め、段階2.5-1 のアーキテクチャ設計で確定する。
...
:tbd_items: TBD-020 再初期化シーケンスの実行中に I2C 送信が失敗した場合の遷移先（エラー状態へ戻るか、シーケンスを最初からやり直すか）は未定。LCD の電源が不安定な状況で現実に発生し得る。TBD-021 rationale は回復試行の契機として表示要求に便乗する設計を許容しているが、SWR_033 はエラー状態の表示要求に即座に失敗を返すことを求めており、便乗により I2C 送信を伴うと両立しない。いずれも段階2.5-1のアーキテクチャ設計で確定する。
