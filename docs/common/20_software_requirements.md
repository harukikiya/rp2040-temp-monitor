...
```{note}
SWR_018 と SWR_019 は欠番。Core0 と Core1 への責務割り当てはソフトウェア要件ではなく
...
:layer: application
:refines: SYS_004
:rationale: busy-wait方式は常にCPUを消費し、消費電力面でも他処理との並行実行面でも不利になる。実現手段としてはハードウェアイベントとWFI(Wait For Interrupt)命令を組み合わせたイベント駆動構造を想定しており、CPUがアイドル時にスリープでき、各イベント処理のフローも分離されて見通しが良くなる。対象となるイベントはタイマー割込、DMA完了割込、コア間通信等。

Core0およびCore1は、処理すべきイベントが無い間、CPUを消費しないこと。
busy-waitによる待機を行わないこと。
```

## SYS_005（コア間データ整合性）からの派生

```{note}
本節の要件は、計算結果（統計値・トレンド結果）のCore1からCore0への受け渡しに関する整合性を対象とする。
リングバッファのCore0からCore1への読み出しはSWR_008の規定に従う。

コア間で共有されるデータには性質の異なる2種類が存在する。
リングバッファ（Core0→Core1、サンプル列）は最大1サンプルの不整合を許容する設計（SWR_008参照）。
詳細はADR-0002参照。

SWR_025 は欠番。上記は段階4で合格・不合格を判定できる記述ではなく承認基準3を
満たさないため、要件から解説に変更した。リナンバーは行わない。
```
...
:tbd_items: TBD-019 連続失敗数のリセット条件（表示要求の成功時にリセットするか、再初期化の完了時にリセットするか）は未定。4回失敗した後に1回成功し再び失敗した場合の遷移が定まらず、閾値の境界を検証できない。欠番である SWR_032 との関係を含め、段階2.5-1 のアーキテクチャ設計で確定する。
...
:tbd_items: TBD-020 再初期化シーケンスの実行中に I2C 送信が失敗した場合の遷移先（エラー状態へ戻るか、シーケンスを最初からやり直すか）は未定。LCD の電源が不安定な状況で現実に発生し得る。TBD-021 rationale は回復試行の契機として表示要求に便乗する設計を許容しているが、SWR_033 はエラー状態の表示要求に即座に失敗を返すことを求めており、便乗により I2C 送信を伴うと両立しない。いずれも段階2.5-1 のアーキテクチャ設計で確定する。
