...
:layer: application
:refines: SYS_004
:rationale: busy-wait方式は常にCPUを消費し、消費電力面でも他処理との並行実行面でも不利になる。実行手段としてはハードウェアイベントとWFI(Wait For Interrupt)命令を組み合わせたイベント駆動構造を想定しており、CPUがアイドル時にスリープでき、各イベント処理のフローも分離されて見通しが良くなる。対象となるイベントはタイマー割込、DMA完了割込、コア間通信等。
